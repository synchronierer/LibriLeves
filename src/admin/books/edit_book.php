...
$message = '';
$redirect = false; // Variable für die Weiterleitung
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_book'])) {
    ...
    // Barcode-Validierung und Vorschläge
    if (!empty($barcode)) {
        ...
        if ($count > 0) {
            // Falls der Barcode bereits vergeben ist, erzeuge einen alternativen Barcode-Vorschlag basierend auf der ISBN
            ...
            // Wenn Barcode nicht vergeben ist, Buch aktualisieren
            ...
            if ($stmt->execute()) {
                ...
                $redirect = true; // Setze die Weiterleitungsvariable
            } else {
        ...
        // Falls kein Barcode eingegeben wurde, Buch aktualisieren ohne Barcode
...
// Buchdaten abrufen, um sie im Formular anzuzeigen
...
<body>
    ...
    <?php if ($message): ?>
    <div class="popup <?php echo ($redirect ? 'success' : 'error'); ?>">
...
<?php endif; ?>
    ...
    <script>
        // Zeige das Popup, wenn eine Nachricht vorhanden ist
        ...
        if (popup) {
            ...
            setTimeout(() => {
                ...
                <?php if ($redirect): ?>
                    // Leite zur view_books.php weiter, wenn gespeichert wurde
                    window.location.href = 'view_books.php';
                <?php endif; ?>
            }, 6000); // Popup nach 6 Sekunden ausblenden
        }
    </script>
</body>
</html>